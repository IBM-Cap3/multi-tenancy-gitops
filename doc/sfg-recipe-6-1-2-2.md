...
1. Modify MQ and DB2 yamls to match the new b2bi new version name space
2. Go to the `2-services` directory:

    ```bash
    cd multi-tenancy-gitops/0-bootstrap/single-cluster/2-services/argocd/instances
    ```
3. find and open "ibm-sfg-mq-prod.yaml" and "ibm-sgf-db2-prod.yaml", then modify all instances of the namespace from "b2bi-prod" to "b2bi-612"
